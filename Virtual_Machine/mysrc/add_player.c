...
/*   Updated: 2018/10/26 14:19:10 by dbauduin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

void			*parse_player(int fd, char *path, t_env *env, t_player *player)
{
    t_header	*header;
    void		*buffer;
    void		*data;
...
void			add_player(t_env *env, char *path)
{
    ...
    data = parse_player(fd, path, env, player);
    header = (t_header*)data;
    close(fd);
    init_player(env, player, header, data);
