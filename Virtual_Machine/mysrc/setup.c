...
/*   Updated: 2018/10/26 18:37:57 by dbauduin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

static void set_number(t_env *env, t_player **player)
{
    int i;
    int n;
...
void			setup(t_env *env, int ac, char **av)
{
    ...
    if (!ft_parrlen((void**)env->player))
        ft_error(0, env);
    set_number(env, env->player);
    sort_player(env->player);
    set_proc(env);
    i = -1;
    while (++i < env->nb_player)
        ft_print(1, "* Player %d, weighing %d bytes, \"%s\" \"%s\"\n", \
                env->player[i]->nbr, env->player[i]->size, env->player[i]->name, \
                env->player[i]->comment);
}
